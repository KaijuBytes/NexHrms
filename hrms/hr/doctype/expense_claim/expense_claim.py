...
import nex
# from nex.accounts.doctype.repost_accounting_ledger.repost_accounting_ledger import (
# 	validate_docs_for_voucher_types,
# )
# from nex.accounts.doctype.sales_invoice.sales_invoice import get_bank_cash_account
# from nex.accounts.general_ledger import make_gl_entries
...
# 		self.set_status()
# 		self.validate_agency_and_department()
...
# 	def validate_agency_and_department(self):
# 		if self.department:
# 			agency = frappe.db.get_value("Department", self.department, "agency")
# 			if agency and self.agency != agency:
# 				frappe.throw(
# 					_("Department {0} does not belong to agency: {1}").format(self.department, self.agency),
# 					exc=MismatchError,
# 				)
...
# 			# payment entry
# 			payment_account = get_bank_cash_account(self.mode_of_payment, self.agency).get("account")
# 			gl_entry.append(
# 				self.get_gl_dict(
# 					{
...
# 				expense.default_account = get_expense_claim_account(expense.expense_type, self.agency)[
# 					"account"
# 				]
...
# def make_bank_entry(dt, dn):
	...
	# default_bank_cash_account = get_default_bank_cash_account(expense_claim.agency, "Bank")
	# if not default_bank_cash_account:
	# 	default_bank_cash_account = get_default_bank_cash_account(expense_claim.agency, "Cash")

	# payable_amount = get_outstanding_amount_for_claim(expense_claim)

	# je = frappe.new_doc("Journal Entry")
	# je.voucher_type = "Bank Entry"
	# je.agency = expense_claim.agency
	...
	# 		"cost_center": nex.get_default_cost_center(expense_claim.agency),
...
@frappe.whitelist()
def get_expense_claim_account_and_cost_center(expense_claim_type, agency):
	data = get_expense_claim_account(expense_claim_type, agency)
...
@frappe.whitelist()
def get_expense_claim_account(expense_claim_type, agency):
	account = frappe.db.get_value(
		"Expense Claim Account", {"parent": expense_claim_type, "agency": agency}, "default_account"
	)
	if not account:
		frappe.throw(
...
def update_outstanding_amount_in_payment_entry(expense_claim: dict, pe_reference: str):
	...
	# TODO: refactor convoluted code after nex payment entry becomes extensible
