...
class EmployeeTransfer(Document):
	...
	def on_submit(self):
		...
		if self.create_new_employee_id:
			...
			if self.new_agency and self.agency != self.new_agency:
				new_employee.internal_work_history = []
				new_employee.date_of_joining = self.transfer_date
				new_employee.agency = self.new_agency
			...
			if self.new_agency and self.agency != self.new_agency:
				employee.agency = self.new_agency
	...
	def on_cancel(self):
		...
		if self.new_agency != self.agency:
			employee.agency = self.agency
		employee.save()
