...
class JobOpening(WebsiteGenerator):
	...
	def validate(self):
		if not self.route:
			self.route = f"jobs/{frappe.scrub(self.agency)}/{frappe.scrub(self.job_title).replace('_', '-')}"
	...
	def validate_current_vacancies(self):
		if not self.staffing_plan:
			staffing_plan = get_active_staffing_plan_details(self.agency, self.designation)
			if staffing_plan:
		...
		if self.staffing_plan and self.planned_vacancies:
			designation_counts = get_designation_counts(self.designation, self.agency, self.name)
