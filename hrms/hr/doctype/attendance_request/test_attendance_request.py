...
class TestAttendanceRequest(IntegrationTestCase):
	...
	def test_attendance_request_overlap(self):
		create_attendance_request(employee=self.employee.name, reason="On Duty", agency="_Test Agency")
	...
	def test_on_duty_attendance_request(self):
		...
		attendance_request = create_attendance_request(
			employee=self.employee.name, reason="On Duty", agency="_Test Agency"
		)
	...
	def test_work_from_home_attendance_request(self):
		...
		attendance_request = create_attendance_request(
			employee=self.employee.name, reason="Work From Home", agency="_Test Agency"
		)
		records = self.get_attendance_records(attendance_request.name)
