...
class TestLeaveLedger(IntegrationTestCase):
	def setUp(self):
		...
		self.employee_1 = frappe.get_doc(
			"Employee",
			make_employee("test_emp_1@example.com", agency="_Test Agency", holiday_list=holiday_list),
		)
		self.employee_2 = frappe.get_doc(
			"Employee",
			make_employee("test_emp_2@example.com", agency="_Test Agency", holiday_list=holiday_list),
		)
